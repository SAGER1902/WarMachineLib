...
import safx.SagerFX;
public class ItemGun extends Item implements Vanishable {
	...
	public boolean zoomHide = false;
	public String aim_tex = null;
	...
	public void inventoryTick(ItemStack stack, Level worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
		...
		if(entityIn instanceof Player){
			Player fireplayer = (Player)entityIn;
			if(isSelected) {
			...
			if(stack.hasTag()) {
			  ...
			  CompoundTag ltags = new CompoundTag();
			  stack.setTag(ltags);
			}
		}
	}
	
	public void gunbase_recoil(ItemStack itemstack, Level world, Entity entity, int i, boolean flag, int ii) {
    ...
    public void gunbase_powertime(ItemStack itemstack, Level world, Player player) {
    	//if(world.isClientSide) 
    	{
    		CompoundTag nbt = itemstack.getTag();
        	//boolean recoiled = nbt.getBoolean("Recoiled");
    		int power_time_nbt = nbt.getInt("PowerTime");
    		{
				...
				/*if(this.power_sound != null && this.powertime%this.power_sound_tick==0){
					world.playSound((Player)null, player.posX, player.posY, player.posZ,
							SoundEvent.REGISTRY.getObject(ResourceLocation.tryParse(this.modid, this.power_sound)), SoundSource.NEUTRAL, 3.0F, 1F);
				}*/
    		}
    	}
    }
    ...
    public void Fire_AR_Left(ItemStack itemstack, Level world, Entity entity, boolean flag, boolean flag1) {//发射
    ...
    public void Fire_SR_Left(ItemStack itemstack, Level world, Entity entity, boolean flag, boolean flag1) {//半自动
    	int li = getMaxDamage() - itemstack.getDamageValue();
    	Player fireplayer = (Player) entity;
		//CompoundTag nbt = itemstack.getTag();
	...
	double recoil1 = 1;
	public void FireBullet(ItemStack stack, Level worldIn, Player playerIn) 
	{
		if(stack.hasTag()) {
		...
		worldIn.playSound(playerIn, playerIn.getX(), playerIn.getY(), playerIn.getZ(),getSound(modid,fire_sound), SoundSource.NEUTRAL, 3.0F, 1.0F);
		{
			recoil1=recoil;
			double xx11 = 0;
			double zz11 = 0;
			...
			playerIn.setXRot(playerIn.getXRot()+(worldIn.random.nextFloat() * -2F) * (float)reco);
			
			double yy = fire_posy;
			if (playerIn.isCrouching()) {
				yy = fire_posy_sneak;
			}
			double zzz = fire_posz*0.1F * Math.cos(Math.toRadians(-playerIn.getXRot()));
			xx11 -= Math.sin(playerIn.yHeadRot * 0.01745329252F) * zzz;
			zz11 += Math.cos(playerIn.yHeadRot * 0.01745329252F) * zzz;
			xx11 -= Math.sin(playerIn.yHeadRot * 0.01745329252F + xz) * fire_posx;
			zz11 += Math.cos(playerIn.yHeadRot * 0.01745329252F + xz) * fire_posx;
			yy11 = Math.sqrt(zzz* zzz) * Math.tan(Math.toRadians(-playerIn.getXRot())) * 1D;
			if(this.bulletid < 10){
				...
				for (int i = 0; i < this.bulletcount; ++i) {
					...
					bullet.moveTo(playerIn.getX() + xx11, playerIn.getY() + yy + yy11, playerIn.getZ() + zz11, playerIn.yHeadRot, playerIn.getXRot());
					bullet.shootFromRotation(playerIn, playerIn.getXRot(), playerIn.yHeadRot, 0.0F, this.bulletspeed, this.bulletspread);
    ...
    public void onFireAnimation(Player player, ItemStack itemstack) {
	...
	public void setReload(ItemStack gun_item, Level par2Level, Player par3Player) 
	{
		//boolean linfinity = EnchantmentHelper.getEnchantmentLevel(Enchantments.INFINITY_ARROWS, par3Player) > 0;
		boolean linfinity = true;
		if(!par3Player.getAbilities().instabuild){//2019/10/27
			for (int i = 0; i < par3Player.getInventory().getContainerSize(); ++i) {
				ItemStack itemstack = par3Player.getInventory().getItem(i);
				/*if (this.isAmmo(itemstack)) */{
					...
					if (!ammo_item.isEmpty()) {
						...
						if (!linfinity) {
							...
							if (ammo_item.isEmpty()) {
								par3Player.getInventory().removeItem(ammo_item);
							}
						}
	...
	public boolean isReload(ItemStack gun_item) {
	 ...
	 public void Gun_Reload(ItemStack itemstack, Level world, Entity entity, int i, boolean flag) {
			CompoundTag nbt = itemstack.getTag();
			if (itemstack.hasTag()/* && this.magazine != null*/) {
				Player Player = (Player)entity;
				ItemGun gun = (ItemGun) itemstack.getItem();
				//if (this.isReload(itemstack) && flag && Player.getInventory().hasItemStack(new ItemStack(this.magazine))) {
				if (this.isReload(itemstack) && flag/* && !this.findAmmo(Player).isEmpty()*/) {
						if (entity != null && entity instanceof Player) {
							if (itemstack == Player.getMainHandItem()) {
								int reloadti = nbt.getInt("RloadTime");
								{
									gun.retime = reloadti;
									if(gun.retime == 0) {
										//SoundEvent sound = new SoundEvent(ResourceLocation.tryParse(WarMachineLib.MOD_ID, reload_sound));
										world.playSound(Player, Player.getX(), Player.getY(), Player.getZ(), 
												this.getSound(modid, this.reload_sound), SoundSource.NEUTRAL, 1.0F, 1.0F);
									}
									++reloadti;
									if (reloadti >= gun.reloadtime) {
										...
										{
											setReload(itemstack, world, Player);
										}
									}else{
   ...
   /**
    * Get the predicate to match ammunition when searching the player's getInventory(), not their main/offhand
    */
