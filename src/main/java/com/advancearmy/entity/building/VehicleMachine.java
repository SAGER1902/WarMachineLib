...
public class VehicleMachine extends TamableAnimal implements IHealthBar, IBuilding{
	public VehicleMachine(EntityType<? extends VehicleMachine> p_i48549_1_, Level p_i48549_2_) {
	  super(p_i48549_1_, p_i48549_2_);
	  this.noCulling = true;
	}
	public VehicleMachine(PlayMessages.SpawnEntity packet, Level worldIn) {
		super(ModEntities.ENTITY_VMAC.get(), worldIn);
	}
	public void checkDespawn() {
	}
	public static AttributeSupplier.Builder createAttributes() {
        return VehicleMachine.createMobAttributes();
    }
	public VehicleMachine getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
	  return null;
	}
   
	private static final EntityDataAccessor<Integer> VehicleC = SynchedEntityData.<Integer>defineId(VehicleMachine.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<Integer> Money = SynchedEntityData.<Integer>defineId(VehicleMachine.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<Integer> Build = SynchedEntityData.<Integer>defineId(VehicleMachine.class, EntityDataSerializers.INT);
	private static final EntityDataAccessor<Integer> YawRote = SynchedEntityData.<Integer>defineId(VehicleMachine.class, EntityDataSerializers.INT);
				...
				if(heldItem.is(ItemTags.PICKAXES)){
					if(player.isCrouching()){
						if(!this.level().isClientSide && this.getHealth()>=this.getMaxHealth()){
							this.dropItemStack(new ItemStack(ModItems.vehicle_machine.get()));
					 ...
					 if(item instanceof ItemSpawn){
						...
						if(this.getVehicleC()==0){
							if(vehicleitem.type == 0){
								...
								player.sendSystemMessage(Component.translatable("advancearmy.interact.machinev.desc"));
		...
		if (this.isAlive()){
			...
			if(this.getMainHandItem()!=null){
				...
				if(item instanceof ItemSpawn){
					...
					AABB axisalignedbb = (new AABB(i1, j1, k1, i1+range, j1+range, k1+range)).inflate(1D);
							...
							/*if(player != null && this.isOwner(player))*/{
								if(this.getMoney()>0){
									if(this.getBuild() < this.finish_time){
										...
										if(this.getBuild()%20==0)this.playSound(SASoundEvent.fix.get(), 1.0F, 1.0F);
									}
								}
							}
						}
