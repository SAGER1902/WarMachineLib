...
public class EntitySA_LAVAA extends EntitySA_LandBase{
	public EntitySA_LAVAA(EntityType<? extends EntitySA_LAVAA> sodier, Level worldIn) {
		...
		this.obj = new SAObjModel("advancearmy:textures/mob/lav25aa.obj");
		seatView3X=0F;
		seatView3Y=-2F;
		seatView3Z=-4.5F;
		...
		this.fireposY1 = 2.82F;
		this.fireposZ1 = 1.46F;
		
		this.firebaseX = 0.24F;
		this.firebaseZ = 0F;
	...
	public EntitySA_LAVAA(PlayMessages.SpawnEntity packet, Level worldIn) {//
		super(ModEntities.ENTITY_LAV.get(), worldIn);
	}
	public static AttributeSupplier.Builder createAttributes() {
        return EntitySA_LAVAA.createMobAttributes().add(Attributes.KNOCKBACK_RESISTANCE, (double) 10.0D)
	...
	public void weaponActive1(){
		if (this.tracktick % 2 == 0)this.playSound(SASoundEvent.fire_minigun.get(), 3.0F, 1.0F);
		//++rote_tick;
		/*if(rote_tick<360){
			++rote_tick;
		}else{
		...
		bullet.setExLevel(1);
		bullet.setModel("advancearmy:textures/entity/bullet/bullet20mm.obj");
		bullet.setTex("advancearmy:textures/entity/bullet/bullet12.7.png");
		...
		bullet.shootFromRotation(this, this.turretPitch, this.turretYaw, 0.0F, 16F, 1);
		if (!this.level().isClientSide) this.level().addFreshEntity(bullet);
	}
	public void weaponActive2(){
		this.playSound(SASoundEvent.fire_missile.get(), 3.0F, 1.0F);
		float fire_x = 0.35F;
		...
		bullet.moveTo(this.getX()+px + xx11, this.getY()+3.1F+base, this.getZ()+pz + zz11, this.getYRot(), this.turretPitch);
