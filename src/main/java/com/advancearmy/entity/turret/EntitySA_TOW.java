...
public class EntitySA_TOW extends EntitySA_TurretBase{
	public EntitySA_TOW(EntityType<? extends EntitySA_TOW> sodier, Level worldIn) {
		...
		this.hud_box_obj = "wmlib:textures/hud/boxtow.obj";
		...
		this.firesound1 = SASoundEvent.fire_tow.get();
		...
		if(this.getTargetType()==0 && this.getOwner()!=null){
			...
			for(int k2 = 0; k2 < list.size(); ++k2) {
				...
				if(ent instanceof EntityMissile){
					/*have=true;
					break;*/
					/*EntityMissile mis = (EntityMissile)ent;
					if((LivingEntity)mis.getOwner()==(LivingEntity)this.getOwner()||((EntityMissile)ent).shooter==(LivingEntity)this.getOwner())*/{
						have=true;
						break;
					}
				}
			}
			this.havemissile=have;
		}
	}
	
	public void weaponActive1(){
		String model = "advancearmy:textures/entity/bullet/towmissile.obj";
