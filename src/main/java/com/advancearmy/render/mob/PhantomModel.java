...
@OnlyIn(Dist.CLIENT)
public class PhantomModel<T extends ERO_Phantom>extends HierarchicalModel<T> {
    ...
    public PhantomModel(ModelPart root) {
        this.root = root;
        ModelPart $$1 = root.getChild("body");
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        float $$6 = ((float)((ERO_Phantom)entity).getUniqueFlapTickOffset() + ageInTicks) * 7.448451f * ((float)Math.PI / 180);
        float $$7 = 16.0f;
