...
@OnlyIn(Dist.CLIENT)
public class RenderConscript extends MobRenderer<EntitySA_Conscript, ModelNone<EntitySA_Conscript>>
{
	...
	public ResourceLocation tex = ResourceLocation.tryParse("advancearmy:textures/mob/soldier/conscript.png");
    private static final SAObjModel obj = new SAObjModel("advancearmy:textures/mob/soldier/conscript.obj");
	
    private static final SAObjModel ak47 = new SAObjModel("advancearmy:textures/gun/ak47.obj");
    private static final ResourceLocation ak47t = ResourceLocation.tryParse("advancearmy:textures/gun/ak47.png");
    private static final SAObjModel ppsh41 = new SAObjModel("advancearmy:textures/gun/ppsh41.obj");

    private static final SAObjModel rpg1 = new SAObjModel("advancearmy:textures/gun/rpg.obj");
    private static final ResourceLocation rpg1t = ResourceLocation.tryParse("advancearmy:textures/gun/rpg.png");
    ...
    private static final SAObjModel fire = new SAObjModel("advancearmy:textures/gun/fm.obj");
    private static final SAObjModel aagun = new SAObjModel("advancearmy:textures/gun/aagun.obj");
	
	private static final ResourceLocation gunt = ResourceLocation.tryParse("advancearmy:textures/gun/gun.png");
	private static final ResourceLocation fire_tex = ResourceLocation.tryParse("advancearmy:textures/entity/flash/muzzleflash3.png");//flash
	
	
	RenderType rt = RenderTypeVehicle.objrender(tex);
	RenderType rtakt = RenderTypeVehicle.objrender(ak47t);
    ...
    public RenderConscript(EntityRendererProvider.Context context)
    {
    	super(context, new ModelNone(),0.5F);
    }

    public ResourceLocation getTextureLocation(EntitySA_Conscript entity)
    {
		return tex;
    }
    private void render_light(EntitySA_Conscript entity, String name){
		obj.renderPart(name);
    }
	
    float iii;
    public void render(EntitySA_Conscript entity, float entityYaw, float partialTicks, PoseStack stack, MultiBufferSource bufferIn, int packedLight)
    {
    ...
    private void renderlegs(PoseStack stack, EntitySA_Conscript entity, float limbSwing, float limbSwingAmount, float partialTicks){
    ...
    private void renderbody(EntitySA_Conscript entity, float limbSwing, float limbSwingAmount, float partialTicks, PoseStack stack, MultiBufferSource bufferIn, int packedLight){
		Minecraft minecraft = Minecraft.getInstance();
		stack.pushPose();
    	obj.renderPart("body");
		if(entity.getWeaponId()==5)obj.renderPart("pyro");
		SAObjModel wait_model = ak47;
		...
		if(entity.mainWeaponId!=0){
			...
			}else if(entity.getWeaponId()==6){//sniper
				stack.mulPose(Axis.XP.rotationDegrees(-90F));
		...
		stack.pushPose();
		{
			...
			if(entity.canfire){
				...
				stack.pushPose();
				stack.scale(0.1875F, 0.1875F, 0.1875F);
				if(!entity.isAttacking()&&(entity.getWeaponId()==3||entity.getWeaponId()==4||entity.getWeaponId()==0)){
